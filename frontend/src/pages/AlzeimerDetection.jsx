...
const AlzheimerDetectionPage = () => {
  ...
  const [mriMessage, setMriMessage] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Check if the response contains MRI classification message
      if (data.message) {
        setMriMessage(data.message);
      }

      // If an MRI, set prediction values
      if (data.prediction) {
